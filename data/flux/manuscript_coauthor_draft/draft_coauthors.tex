...
\begin{itemize}
  \item Again the existing report figures, unchanged: M4, 12-month window, \textbf{natural-breaks} thresholds
  ...
  \item \textbf{These figures still include uWUE}, since they are taken unchanged from the report. If uWUE is
        dropped everywhere, they need regenerating without it.
  ...
  \item \decide{regenerate without uWUE for consistency with the rest of the draft?}
\end{itemize}
...
\begin{itemize}
  ...
  \item \textbf{Known issue:} the random-forest Optuna SHAP table lost the names of the 14 trait variables whose
        ...
        been reassigned to Traits in the summary percentages quoted above. Figures 3 and 4 are taken unchanged from the report, so the random-forest panels of Figure 4 still carry the unfixed grouping; the extraction in \texttt{run\_v10\_RF\_optuna\_SHAP.R} should be fixed at source.
