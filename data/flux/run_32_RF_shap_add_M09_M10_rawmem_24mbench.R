...
# Output: RF_outputs_rawmem_24mbench/RF_site_shap_M04_M08.csv
# ============================================================
...
# IMPORTANT: refresh all_cols now that the raw lag EFP columns exist,
# otherwise the model_specs %in% all_cols filter below silently drops them
all_cols <- names(dt_rf)
...
model_specs <- list(
  M09_12m = unique(efp_mem_12m),
  M09_24m = unique(efp_mem_24m),
  M10_12m = unique(c(efp_mem_12m, dist_12m)),
  M10_24m = unique(c(efp_mem_24m, dist_24m))
)
...
out_path <- file.path(out_dir, "RF_site_shap_M09_M10.csv")
