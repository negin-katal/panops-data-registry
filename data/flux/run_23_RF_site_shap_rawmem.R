...
# ============================================================
# RUN 23: Per-site SHAP values — RAW-LAG EFP memory version
#         M04_12m (C+T+D) and M08_12m (C+T+D+raw-lag memory)
#
# Mirrors run_20_RF_LOSO_rawmem_fixed.R exactly for predictor
# construction, then runs TreeSHAP per LOSO fold.
#
# Output: RF_outputs_rawmem_fixed_v2/RF_site_shap_M04_M08.csv
# ============================================================
...
out_dir    <- "derived_tables/outputs_afterEGU_results/RF_outputs_rawmem_fixed_v2"
...
# ============================================================
# 2) Pre-CGS rolling 12-month climate summaries
# ============================================================
...
# ============================================================
# 2b) Raw lagged EFP columns (lag1 only for 12m window)
# ============================================================
...
for (efp in efp_vars) {
  ...
  dt_rf <- merge(dt_rf,
                 lag1_lut[, c("SITE_ID", "YEAR", lag1_col), with = FALSE],
                 by = c("SITE_ID", "YEAR"), all.x = TRUE)
}

raw_lag1_cols <- paste0(efp_vars, "_lag1_raw")
cat("Raw lag1 columns:", paste(raw_lag1_cols, collapse = ", "), "\n")
...
# ============================================================
# 4) Benchmark set (complete cases across M04 + M08)
# ============================================================

all_preds  <- unique(c(meteo_12m, trait_vars, dist_12m, efp_mem_12m))
bench_mask <- complete.cases(dt_rf[, ..all_preds])
dt_bench   <- dt_rf[bench_mask]
cat("Benchmark:", nrow(dt_bench), "rows,", uniqueN(dt_bench$SITE_ID), "sites\n")
...
# ============================================================
# 6) Model specs: M04 and M08 only
# ============================================================
...
# ============================================================
# 8) Combine, add group labels, save
# ============================================================
