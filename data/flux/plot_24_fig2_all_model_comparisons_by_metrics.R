...
OUT_DIR <- "plots/manuscript_candidates/fig2_per_metric_models"
...
make_metric_panel_fig2 <- function(m1_key, m2_key, metric_col, metric_name, metric_label, tag_letter,
                                    m1_label, m2_label) {
  # Filter data for this model pair
  data <- site_rmse[model_key %in% c(m1_key, m2_key), ]

  # Pivot to get both models
  data_wide <- dcast(data, SITE_ID + response ~ model_key, value.var = "rmse")

  # Merge with site metrics
  data_wide <- merge(data_wide, site_metrics, by = "SITE_ID")
  data_wide[, response := factor(response, levels = EFP_ORDER)]

  # Long format for plotting
  data_long <- melt(data_wide,
    id.vars = c("SITE_ID", "response", metric_col),
    measure.vars = c(m1_key, m2_key),
    variable.name = "model_key",
    value.name = "site_rmse"
  )

  data_long[, model_type := ifelse(model_key == m1_key, m1_label, m2_label)]
  data_long[, model_type := factor(model_type, levels = c(m1_label, m2_label))]
  data_long[, response := factor(response, levels = EFP_ORDER)]
  data_long[, metric := get(metric_col)]

  # Create color mapping
  color_map <- setNames(c(COL_BASE, COL_DIST), c(m1_label, m2_label))

  p <- ggplot(data_long, aes(x = metric, y = site_rmse, fill = model_type, group = interaction(metric, model_type))) +
    geom_violin(trim = TRUE, scale = "width", width = 0.75, colour = NA, alpha = 0.9,
...
# ── 5) Model comparison definitions ──────────────────────────────────────────
model_comparisons <- list(
  list(m1 = "M01", m2 = "M02", title_short = "C_vs_CplusD",
       m1_label = "C", m2_label = "C+D", title_long = "Climate vs Climate+Disturbance (M01 vs M02)"),
  list(m1 = "M03", m2 = "M04", title_short = "CplusT_vs_CTplusD",
       m1_label = "C+T", m2_label = "C+T+D", title_long = "Climate+Traits vs Climate+Traits+Disturbance (M03 vs M04)"),
  list(m1 = "M05", m2 = "M06", title_short = "CplusM_vs_CplusDplusM",
       m1_label = "C+M", m2_label = "C+D+M", title_long = "Climate+Meteo vs Climate+Disturbance+Meteo (M05 vs M06)"),
  list(m1 = "M07", m2 = "M08", title_short = "CTplusM_vs_CTplusDplusM",
       m1_label = "C+T+M", m2_label = "C+T+D+M", title_long = "Climate+Traits+Meteo vs Climate+Traits+Disturbance+Meteo (M07 vs M08)")
)

# ── 6) Generate plots for each model comparison ──────────────────────────────
for (comp in model_comparisons) {
  cat("\n=== Generating:", comp$title_long, "===\n")

  # Create three metric panels
  p_abs <- make_metric_panel_fig2(comp$m1, comp$m2, "abs_mort_tier",
                                   "Absolute Mortality", "Absolute Mortality (%)", "a",
                                   comp$m1_label, comp$m2_label)
  p_rel <- make_metric_panel_fig2(comp$m1, comp$m2, "rel_mort_tier",
                                   "Relative Mortality", "Relative Mortality (%)", "b",
                                   comp$m1_label, comp$m2_label)
  p_dist <- make_metric_panel_fig2(comp$m1, comp$m2, "rel_dist_tier",
                                    "Relative Disturbance", "Relative Disturbance (%)", "c",
                                    comp$m1_label, comp$m2_label)

  # Combine into single figure
  fig_combined <- (p_abs / p_rel / p_dist) +
    plot_annotation(
      title = paste0("RMSE by mortality/disturbance metrics: ", comp$title_long),
      subtitle = paste0(comp$m1_label, " (blue) vs ", comp$m2_label, " (red) | Anomaly 24m window | All 5 EFPs"),
      theme = theme(
        plot.background = element_rect(fill = DARK_BG, colour = NA),
  ...
  # Save figure
  stem <- file.path(OUT_DIR, paste0("fig2_", comp$title_short, "_by_metrics"))
...
cat("\n=== All model comparison plots saved to", OUT_DIR, "===\n")
