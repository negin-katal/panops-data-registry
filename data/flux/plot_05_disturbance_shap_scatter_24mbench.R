...
shap_sources <- list(
  anomaly = "derived_tables/outputs_afterEGU_results/RF_outputs_anomaly_24mbench/RF_site_shap_M04_M08.csv",
  rawlag  = "derived_tables/outputs_afterEGU_results/RF_outputs_rawmem_24mbench/RF_site_shap_M04_M08.csv"
)
...
for (mem_type in names(shap_sources)) {
  ...
  shap_file2 <- sub("RF_site_shap_M04_M08\\.csv$", "RF_site_shap_M09_M10.csv", shap_file)
  if (file.exists(shap_file2)) {
    shap_raw2 <- fread(shap_file2)
    shap_raw  <- rbindlist(list(shap_raw, shap_raw2), fill = TRUE)
  }
  cat("\nLoaded", mem_type, "SHAP:", nrow(shap_raw), "rows (incl. M09/M10 if present)\n")
  ...
  for (mod_key in c("M04", "M08", "M10")) {
    for (resp in c("GPPsat", "NEPmax", "ETmax", "uWUE")) {
      ...
      pred_lab <- if (mod_key == "M04") "C + T + D" else
                  if (mod_key == "M10") sprintf("M (%s) + D", mem_lab) else
