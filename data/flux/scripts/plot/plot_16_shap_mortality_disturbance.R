...
shap_pct <- merge(shap_pct, site_metrics[, .(SITE_ID, abs_mort_tier, rel_mort_tier, rel_dist_tier)], by = "SITE_ID")
...
make_shap_plot <- function(tier_col, tier_name, x_label) {
  ...
  p <- ggplot(data, aes(x = tier, y = dist_pct, fill = tier)) +
    ...
    labs(
      title = paste0("SHAP contribution of Disturbance group by ", tier_name, " (M04, 24m)"),
...
stem <- file.path(OUT_DIR, "fig_threshold_SHAP_mortality_disturbance")
