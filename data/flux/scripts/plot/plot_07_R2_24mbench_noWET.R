...
# ============================================================
# PLOT 07 (noWET): R² bar comparison — with vs without disturbance
#          Dark theme. WET sites excluded (post-hoc, 128 sites).
#          Uses the unified 24m benchmark (same rows for 12m/24m).
#
...
metrics_sources <- list(
  anomaly = "derived_tables/outputs_afterEGU_results/RF_outputs_24mbench_noWET_posthoc/metrics_anomaly_noWET.csv",
  rawlag  = "derived_tables/outputs_afterEGU_results/RF_outputs_24mbench_noWET_posthoc/metrics_rawlag_noWET.csv"
)
...
model_pairs <- list(
  list(base = "M01", dist = "M02", lbl_base = "C",         lbl_dist = "C + D"),
  list(base = "M03", dist = "M04", lbl_base = "C + T",     lbl_dist = "C + T + D"),
  list(base = "M05", dist = "M06", lbl_base = "C + M",     lbl_dist = "C + D + M"),
  list(base = "M07", dist = "M08", lbl_base = "C + T + M", lbl_dist = "C + T + D + M")
)
...
make_r2_bar <- function(dt_pair, pair, window_str, mem_type, n_sites) {
  ...
  title_str <- sprintf(
    "R² comparison: %s vs %s   [%s window, %s, no WET, n=%d sites]",
    pair$lbl_base, pair$lbl_dist, window_str, mem_title[mem_type], n_sites
  )
...
for (mem_type in names(metrics_sources)) {
  ...
  for (win in c("12m", "24m")) {
    ...
    for (pair in model_pairs) {
      ...
      n_sites <- met_w[model_base == pair$dist, n_sites][1]

      p <- make_r2_bar(dt_pair, pair, win, mem_type, n_sites)

      fname <- sprintf("R2_comparison_%s_vs_%s_%s_%s_noWET",
