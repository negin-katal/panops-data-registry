#!/usr/bin/env Rscript
# RF M01-M08: v4 Lag1 only + Raw-lag memory
...
out_dir <- "derived_tables/outputs_afterEGU_results/RF_outputs_v4_lag1_rawmem"
...
cat("📊 RF LOSO: v4 Lag1 + Anomaly Memory\n")
cat("=====================================\n\n")
...
dt <- fread('derived_tables/outputs_afterEGU_results/V4_combined/EFP_meteo_mortality_traits_v4_RF_benchmark_lag1only.csv')
...
all_dist_cols <- grep("^(mortality|deadwood|forest_loss|new_mortality).*_100m$", names(dt), value = TRUE)
all_dist_cols <- setdiff(all_dist_cols, grep("_lag", all_dist_cols, value = TRUE))
meteo_cols <- grep("^(TA|VPD|P|SW_IN)_.*_cgs$", names(dt), value = TRUE)
efp_rawlag_cols <- grep("^(GPPsat|NEPmax|ETmax|uWUE|WUE)_lag1$", names(dt), value = TRUE)

cat("Feature sets:\n")
cat("  Traits: ", length(all_trait_cols), "\n")
cat("  Disturbance: ", length(all_dist_cols), "\n")
cat("  Meteo + lag1: ", length(meteo_cols), "\n")
cat("  EFP memory (lag1): ", length(efp_rawlag_cols), "\n\n")
...
for (resp_var in RESPONSE_VARS) {
  ...
  model_configs <- list(
    ...
    M05 = c(meteo_cols, efp_rawlag_cols),
    M06 = c(meteo_cols, all_dist_cols, efp_rawlag_cols),
    M07 = c(meteo_cols, all_trait_cols, efp_rawlag_cols),
    M08 = c(meteo_cols, all_trait_cols, all_dist_cols, efp_rawlag_cols)
  )
...
cat("\n✅ v4 Lag1 Anomaly Complete!\n")
cat("Sites:", uniqueN(all_preds$SITE_ID), "| Output:", out_dir, "\n")
