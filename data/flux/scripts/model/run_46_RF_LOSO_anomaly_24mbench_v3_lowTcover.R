...
# Based on run_39_RF_LOSO_anomaly_24mbench_v3.R (July 3, 2026)
...
out_dir    <- "derived_tables/outputs_afterEGU_results/V3_outputs/RF_outputs_lowTcover_anomaly_24mbench"
...
# EFP memory anomalies
efp_anom_12m <- grep("^(GPPsat|NEPmax|ETmax|uWUE|WUE)_anom_lag", names(dt_rf), value = TRUE)
efp_anom_12m <- efp_anom_12m[!grepl("_lag2", efp_anom_12m)]

efp_anom_24m <- grep("^(GPPsat|NEPmax|ETmax|uWUE|WUE)_anom_lag", names(dt_rf), value = TRUE)
...
cat("EFP mem 12m:", length(efp_anom_12m), "\n")
cat("EFP mem 24m:", length(efp_anom_24m), "\n")

# =========================================================
# 4) M01–M08: 12m and 24m windows, anomaly memory
# =========================================================
...
for (resp_var in RESPONSE_VARS) {
  ...
  for (window in c("12m", "24m")) {
    ...
    efp_mem_cols <- if (window == "12m") efp_anom_12m else efp_anom_24m
...
cat("\n✅ LOW TREE COVER FILTERING: Complete!\n")
