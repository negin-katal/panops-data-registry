#!/usr/bin/env Rscript
# RF LOSO: Benchmark 2 (lag1+lag2) - Final Pipeline
...
cat('\n====================================================================\n')
cat('RF LOSO: Benchmark 2 (Lag1+Lag2) - Final Pipeline\n')
cat('====================================================================\n\n')
...
cat('Loading Benchmark 2 (lag1+lag2)...\n')

dt <- fread('derived_tables/outputs_afterEGU_results/V9_final_benchmarks/benchmark_B2_lag1and2_final.csv')
...
dist_cols_cur <- grep('^(forest_mean|deadwood_mean|mortality_intensity)_500m$', names(dt), value=TRUE)
dist_cols_lag1 <- grep('^(forest_mean|deadwood_mean|mortality_intensity)_500m_lag1$', names(dt), value=TRUE)
dist_cols_lag2 <- grep('^(forest_mean|deadwood_mean|mortality_intensity)_500m_lag2$', names(dt), value=TRUE)
meteo_cols_cur <- grep('^(TA|VPD|P|SW_IN)_', names(dt), value=TRUE)
meteo_cols_lag12 <- grep('^(TA|VPD|P|SW_IN)_.*_(lag1|lag2)$', names(dt), value=TRUE)
meteo_cols_cur <- setdiff(meteo_cols_cur, meteo_cols_lag12)

# Raw lag memory (EFP lag1 and lag2 values)
efp_lag_raw <- grep('^(GPPsat|NEPmax|ETmax|uWUE)_lag[12]$', names(dt), value=TRUE)
# Anomaly memory (EFP lag1 and lag2 anomalies)
efp_lag_anom <- grep('^(GPPsat|NEPmax|ETmax|uWUE)_anom_lag[12]$', names(dt), value=TRUE)

cat('Feature set summary:\n')
cat('  Traits:', length(trait_cols), '\n')
cat('  Disturbance (current):', length(dist_cols_cur), '\n')
cat('  Disturbance (lag1):', length(dist_cols_lag1), '\n')
cat('  Disturbance (lag2):', length(dist_cols_lag2), '\n')
cat('  Meteorology (current):', length(meteo_cols_cur), '\n')
cat('  Meteorology (lag1+lag2):', length(meteo_cols_lag12), '\n')
...
for (mem_type in memory_types) {
  ...
  model_configs <- list(
    M01 = c(meteo_cols_cur, meteo_cols_lag12),
    M02 = c(meteo_cols_cur, meteo_cols_lag12, dist_cols_cur, dist_cols_lag1, dist_cols_lag2),
    M03 = c(meteo_cols_cur, meteo_cols_lag12, trait_cols),
    M04 = c(meteo_cols_cur, meteo_cols_lag12, trait_cols, dist_cols_cur, dist_cols_lag1, dist_cols_lag2),
    M05 = c(meteo_cols_cur, meteo_cols_lag12, efp_mem),
    M06 = c(meteo_cols_cur, meteo_cols_lag12, dist_cols_cur, dist_cols_lag1, dist_cols_lag2, efp_mem),
    M07 = c(meteo_cols_cur, meteo_cols_lag12, trait_cols, efp_mem),
    M08 = c(meteo_cols_cur, meteo_cols_lag12, trait_cols, dist_cols_cur, dist_cols_lag1, dist_cols_lag2, efp_mem)
  )
...
out_dir <- file.path(OUT_BASE, 'RF_B2_lag1and2_final')
...
cat('====================================================================\n')
cat('SUMMARY - BENCHMARK 2 (LAG1+LAG2)\n')
cat('====================================================================\n\n')
