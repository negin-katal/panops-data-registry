...
# =========================================================
# LEAVE-ONE-SITE-OUT RF — EFP MEMORY AS RAW LAG VALUES
#
# Identical to run_18_RF_LOSO.R except the memory predictor
# group (M05–M08) uses raw EFP values from the preceding
# year(s) instead of z-score anomalies.
#
# efp_mem_12m : GPPsat_lag1, NEPmax_lag1, ETmax_lag1, uWUE_lag1
# efp_mem_24m : same + _lag2 for all four EFPs
#
# Output goes to RF_outputs_rawmem/ so both runs coexist.
# =========================================================
...
out_dir    <- "derived_tables/outputs_afterEGU_results/RF_outputs_rawmem"
...
# =========================================================
# 2) Compute pre-CGS rolling climate summaries
# =========================================================
...
# =========================================================
# 2b) Compute raw lagged EFP columns
#
# For each (SITE_ID, YEAR), look up the EFP value at
# (SITE_ID, YEAR-1) and (SITE_ID, YEAR-2) via a self-join.
# =========================================================

efp_vars <- c("GPPsat", "NEPmax", "ETmax", "uWUE")

# Build a lookup table: SITE_ID + YEAR → EFP values
efp_lookup <- unique(dt_rf[, c("SITE_ID", "YEAR", efp_vars), with = FALSE])

for (efp in efp_vars) {
  # lag1: value at YEAR-1
...
raw_lag1_cols <- paste0(efp_vars, "_lag1_raw")
raw_lag2_cols <- paste0(efp_vars, "_lag2_raw")

cat("\nRaw lag EFP columns created:\n")
cat("  lag1:", paste(raw_lag1_cols, collapse = ", "), "\n")
cat("  lag2:", paste(raw_lag2_cols, collapse = ", "), "\n")
cat("  NA counts lag1:", sapply(raw_lag1_cols, function(x) sum(is.na(dt_rf[[x]]))), "\n")
cat("  NA counts lag2:", sapply(raw_lag2_cols, function(x) sum(is.na(dt_rf[[x]]))), "\n")
...
# Raw EFP lag values (replaces anomaly columns)
efp_mem_12m <- raw_lag1_cols                          # lag1 only  (4 vars)
efp_mem_24m <- c(raw_lag1_cols, raw_lag2_cols)        # lag1+lag2  (8 vars)
...
cat(sprintf("  efp_mem_12m  : %d  (raw lag1 EFPs)\n",      length(efp_mem_12m)))
cat(sprintf("  efp_mem_24m  : %d  (raw lag1+lag2 EFPs)\n", length(efp_mem_24m)))
...
cat("\n--- Effect of adding raw EFP memory (M01 vs M05, 12m) ---\n")
print(all_metrics[
  grepl("M01|M05", model_base) & window == "12m",
  .(model, response, RMSE = round(RMSE, 4), R2 = round(R2, 3))
][order(response, model)])

cat("\n--- Memory effect: anomaly (orig) vs raw lag comparison note ---\n")
cat("  Original anomaly results are in RF_outputs/\n")
cat("  Raw lag results are in RF_outputs_rawmem/\n")
