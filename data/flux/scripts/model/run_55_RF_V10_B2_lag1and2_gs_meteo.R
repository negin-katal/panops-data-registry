#!/usr/bin/env Rscript
# RF LOSO: Benchmark 2 (lag1+lag2) with GS-to-GS meteorological lags
...
cat('\n====================================================================\n')
cat('RF LOSO: Benchmark 2 (Lag1+Lag2, GS-to-GS Meteo) - V10 Pipeline\n')
cat('====================================================================\n\n')
...
cat('Loading Benchmark 2 (lag1+lag2, GS-to-GS meteo)...\n')

dt <- fread('derived_tables/outputs_afterEGU_results/V10_gs_meteo_benchmarks/benchmark_B2_lag1and2_gs_meteo.csv')
...
mort_cols_lag2 <- grep('forest_mean_pct_500m_lag2$|deadwood_mean_pct_500m_lag2$|mortality_intensity_pct_500m_lag2$', names(dt), value=TRUE)
...
cat('  Disturbance (lag2):', length(mort_cols_lag2), '\n')
...
for (mem_type in memory_types) {
  ...
  model_configs <- list(
    ...
    M02 = c(meteo_cols_cur, meteo_cols_lag12, mort_cols_cur, mort_cols_lag1, mort_cols_lag2),
    M03 = c(meteo_cols_cur, meteo_cols_lag12, trait_cols),
    M04 = c(meteo_cols_cur, meteo_cols_lag12, trait_cols, mort_cols_cur, mort_cols_lag1, mort_cols_lag2),
    M05 = c(meteo_cols_cur, meteo_cols_lag12, efp_mem),
    M06 = c(meteo_cols_cur, meteo_cols_lag12, mort_cols_cur, mort_cols_lag1, mort_cols_lag2, efp_mem),
    M07 = c(meteo_cols_cur, meteo_cols_lag12, trait_cols, efp_mem),
    M08 = c(meteo_cols_cur, meteo_cols_lag12, trait_cols, mort_cols_cur, mort_cols_lag1, mort_cols_lag2, efp_mem)
  )
...
out_dir <- file.path(OUT_BASE, 'RF_V10_B2_lag1and2')
...
cat('====================================================================\n')
cat('SUMMARY - BENCHMARK 2 (LAG1+LAG2, GS-TO-GS METEO)\n')
cat('====================================================================\n\n')
...
cat('✅ BENCHMARK 2 RF COMPLETE (V10 with GS-to-GS meteo)!\n')
cat('====================================================================\n\n')
