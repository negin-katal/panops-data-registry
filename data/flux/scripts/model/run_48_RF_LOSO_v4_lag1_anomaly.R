#!/usr/bin/env Rscript
# RF M01-M08: v4 Lag1 only + Anomaly memory
...
out_dir <- "derived_tables/outputs_afterEGU_results/RF_outputs_v4_lag1_anomaly"
...
efp_anom_cols <- grep("^(GPPsat|NEPmax|ETmax|uWUE|WUE)_lag1$", names(dt), value = TRUE)

cat("Feature sets:\n")
cat("  Traits: ", length(all_trait_cols), "\n")
cat("  Disturbance: ", length(all_dist_cols), "\n")
cat("  Meteo + lag1: ", length(meteo_cols), "\n")
cat("  EFP memory (lag1): ", length(efp_anom_cols), "\n\n")
