# v2 — 2026-06-30: rerun with fixed data (negative uWUE/WUE→NA, CN-Sb1/2 outliers→NA, US-HB4 P anomalies→NA)
library(data.table)
library(ranger)
...
out_dir    <- "derived_tables/outputs_afterEGU_results/RF_outputs_rawmem_24mbench_v3"
dir.create(out_dir, recursive = TRUE, showWarnings = FALSE)

RESPONSE_VARS <- c("NEPmax")
N_TREES       <- 500
SEED          <- 42
...
efp_mem_24m <- c(raw_lag1_cols, raw_lag2_cols)
...
cat(sprintf("  efp_mem_12m  : %d  (lag1 raw)\n",      length(efp_mem_12m)))
cat(sprintf("  efp_mem_24m  : %d  (lag1+lag2 raw)\n", length(efp_mem_24m)))
...
cat("  Raw-lag results saved in RF_outputs_rawmem_24mbench/\n")
