#!/usr/bin/env Rscript
# RF LOSO: Benchmark 1 (lag1 only) - Final Pipeline
...
cat('\n====================================================================\n')
cat('RF LOSO: Benchmark 1 (Lag1 Only) - Final Pipeline\n')
cat('====================================================================\n\n')
...
cat('Loading Benchmark 1 (lag1 only)...\n')

dt <- fread('derived_tables/outputs_afterEGU_results/V9_final_benchmarks/benchmark_B1_lag1only_final.csv')
...
meteo_cols_cur <- setdiff(meteo_cols_cur, meteo_cols_lag1)  # Current only
...
for (mem_type in memory_types) {
  ...
  model_configs <- list(
    ...
    M02 = c(meteo_cols_cur, meteo_cols_lag1, dist_cols_cur, dist_cols_lag1),
    M03 = c(meteo_cols_cur, meteo_cols_lag1, trait_cols),
    M04 = c(meteo_cols_cur, meteo_cols_lag1, trait_cols, dist_cols_cur, dist_cols_lag1),
    M05 = c(meteo_cols_cur, meteo_cols_lag1, efp_mem),
    M06 = c(meteo_cols_cur, meteo_cols_lag1, dist_cols_cur, dist_cols_lag1, efp_mem),
    M07 = c(meteo_cols_cur, meteo_cols_lag1, trait_cols, efp_mem),
    M08 = c(meteo_cols_cur, meteo_cols_lag1, trait_cols, dist_cols_cur, dist_cols_lag1, efp_mem)
  )
...
out_dir <- file.path(OUT_BASE, 'RF_B1_lag1_final')
...
cat('====================================================================\n')
cat('SUMMARY - BENCHMARK 1 (LAG1 ONLY)\n')
cat('====================================================================\n\n')
