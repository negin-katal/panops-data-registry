#!/usr/bin/env Rscript
# RF M01-M08: v4 Lag1+Lag2 + Raw-lag memory
...
out_dir <- "derived_tables/outputs_afterEGU_results/RF_outputs_v4_lag1and2_rawmem"
...
cat("📊 RF LOSO: v4 Lag1+Lag2 + Raw-lag Memory\n")
cat("=========================================\n\n")
...
efp_rawlag_cols <- grep("^(GPPsat|NEPmax|ETmax|uWUE|WUE)_lag[12]$", names(dt), value = TRUE)

cat("Feature sets: Traits", length(all_trait_cols), "| Dist", length(all_dist_cols), "| Meteo", length(meteo_cols), "| EFP mem", length(efp_rawlag_cols), "\n\n")
