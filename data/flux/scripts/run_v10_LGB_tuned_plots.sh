#!/usr/bin/env bash
# ============================================================================
# Regenerate every RF-dependent V10 figure from the LightGBM (tuned) outputs.
...
export V10_PREFIX="LGB"
export V10_OUT_ROOT="plots/V10/LightGBM_tuned"

DATASETS=${1:-"all_sites filtered"}

for DS in $DATASETS; do
  if [ "$DS" = "all_sites" ]; then
    export V10_IN_DIR="derived_tables/outputs_afterEGU_results/LGB_v10_all_sites_tuned"
  else
    export V10_IN_DIR="derived_tables/outputs_afterEGU_results/LGB_v10_tuned"
  fi

  echo "############ LightGBM (tuned) plots: $DS ############"
  echo "  in : $V10_IN_DIR"
