...
if (dataset_type == "filtered") {
  ...
  output_base <- 'derived_tables/outputs_afterEGU_results/XGB_v10_tc50_tuned'
...
TUNED <- local({
  ...
  setNames(split(b[, .(max_depth,min_child_weight)], b$response), b$response)
})
...
for (resp in RESPONSE_VARS) {
  ...
  for (model_id in names(model_specs)) {
    ...
    pars <- XGB_PARAMS
    if (!is.null(TUNED[[resp]])) { pars$max_depth <- TUNED[[resp]]$max_depth
