...
# --- per-response hyperparameters, selected by OPTUNA (TPE) ------------------
# scripts/optuna_tune.py -> plots/V10/Optuna/optuna_best_configs.csv
# Optuna was the SAMPLER only; every trial was scored by the same grouped 5-fold
# CV objective (disjoint site groups, seed 42) evaluated by this same R code, so
# nothing leaked into the reported LOSO metric and no cross-language parameter
# transfer was involved. Search space was deliberately wider than the exhaustive
# grids (4 dims for RF, 7 for the boosters).
OPTUNA <- local({
  cfg <- fread("plots/V10/Optuna/optuna_best_configs.csv")
  cfg <- cfg[learner == "LGB"]
  setNames(split(cfg, cfg$response), cfg$response)
})
cat("Optuna-selected config per response:\n")
for (r in names(OPTUNA)) {
  o <- OPTUNA[[r]]
  cat(sprintf("  %-7s %s\n", r,
      paste(sprintf("%s=%s", setdiff(names(o), c("learner","response","best_cv","n_trials")),
                    unlist(o[1, setdiff(names(o), c("learner","response","best_cv","n_trials")), with=FALSE])),
            collapse=" ")))
}
...
cat("\n", strrep("=", 80), "\n", sep="")
cat(sprintf("V10 LIGHTGBM TRAINING (LOSO): %s%s\n", toupper(dataset_type),
...
for (resp in RESPONSE_VARS) {
  ...
  for (model_id in names(model_specs)) {
    ...
    fold_list <- mclapply(site_ids, function(test_site) {
      ...
      o <- OPTUNA[[resp]]
      pars <- modifyList(LGB_PARAMS, list(
        learning_rate = o$learning_rate, num_leaves = as.integer(o$num_leaves),
        min_data_in_leaf = as.integer(o$min_data_in_leaf),
        feature_fraction = o$feature_fraction, bagging_fraction = o$bagging_fraction,
        lambda_l2 = o$lambda_l2))
      bst <- tryCatch(
        lgb.train(params = pars,
                  data    = lgb.Dataset(Xtr, label = train_cc[[resp]]),
                  nrounds = as.integer(o$nrounds),
                  verbose = -1),
...
cat(strrep("=", 80), "\n")
cat("V10 LIGHTGBM TRAINING COMPLETE:", output_base, "\n")
cat(strrep("=", 80), "\n\n")
