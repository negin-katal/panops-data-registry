#!/usr/bin/env bash
# ============================================================================
# Regenerate every RF-dependent V10 figure from the XGBoost outputs.
...
#
#   bash scripts/run_v10_XGB_plots.sh [all_sites|filtered]   (default: both)
# ============================================================================
set -uo pipefail
cd /mnt/gsdata/projects/panops/panops-data-registry/data/flux

RSCRIPT=/home/nk1125/miniconda3/envs/clean_r_env/bin/Rscript
export V10_PREFIX="XGB"
export V10_OUT_ROOT="plots/V10/XGBoost_tuned"

DATASETS=${1:-"all_sites filtered"}

for DS in $DATASETS; do
  if [ "$DS" = "all_sites" ]; then
    export V10_IN_DIR="derived_tables/outputs_afterEGU_results/XGB_v10_all_sites_tuned"
  else
    export V10_IN_DIR="derived_tables/outputs_afterEGU_results/XGB_v10_tuned"
  fi

  echo "############ XGBoost plots: $DS ############"
  echo "  in : $V10_IN_DIR"
