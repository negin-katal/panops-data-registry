...
# per-response CV-selected complexity (must match run_v10_XGB_tuned.R)
TUNED <- local({
  R <- fread("plots/V10/Optuna/optuna_best_configs.csv")[learner == "XGB"]
...
if (SMOKE) {
  ...
  model_specs   <- model_specs["M4_12m"]
}
...
for (resp in RESPONSE_VARS) {
  ...
  model_specs <- model_specs[names(model_specs) %in% c("M4_12m","M4_24m")]  # DRAFT
  ...
  for (model_id in names(model_specs)) {
    ...
    pars <- XGB_PARAMS
    if (!is.null(TUNED[[resp]])) { pars$max_depth <- as.integer(TUNED[[resp]]$max_depth)
                                   pars$min_child_weight <- TUNED[[resp]]$min_child_weight
                                   pars$learning_rate <- TUNED[[resp]]$learning_rate
                                   pars$subsample <- TUNED[[resp]]$subsample
                                   pars$colsample_bytree <- TUNED[[resp]]$colsample_bytree
                                   pars$reg_lambda <- TUNED[[resp]]$reg_lambda }
    bst <- tryCatch(
      xgb.train(params = pars, data = xgb.DMatrix(Xfull, label = df_model[[resp]]),
                nrounds = as.integer(TUNED[[resp]]$nrounds), verbose = 0),
    ...
    fold_shap <- mclapply(sites_test, function(test_site) {
      ...
      # DRAFT: keep the SIGN as well as the magnitude. Per site-year, SUM the signed SHAP
      # within each predictor group (its net push on the prediction, in response units),
      # then average over the site years. Averaging |SHAP| per variable first, as the
      # production script does, discards the direction irrecoverably.
      grp <- vapply(names(shap_mat), get_predictor_group, character(1))
      per_row <- rbindlist(lapply(unique(grp), function(g) {
        cols <- names(shap_mat)[grp == g]
        data.table(group  = g,
                   signed = rowSums(shap_mat[, ..cols], na.rm = TRUE),
                   absol  = rowSums(abs(shap_mat[, ..cols]), na.rm = TRUE))
      }))
      shap_long <- per_row[, .(mean_signed_shap = mean(signed, na.rm = TRUE),
...
if (length(shap_results) > 0) {
  ...
  # group is assigned per row during aggregation; no `variable` column exists here

  out_path <- file.path(output_base,
    "XGB_site_signed_shap_M4.csv")   # DRAFT: never the production file
