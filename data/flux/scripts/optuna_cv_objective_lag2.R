...
#   - evaluated on M2_24m (C+D, no memory, weak diffuse signal) and
#     M6_raw_24m (C+D+M raw, one dominant predictor), then averaged, so a single
#     configuration has to serve both regimes
#
# LAG2 VARIANT: tunes on the 24-month window of the lag2-enabled datasets
# (B2, climate = lag1_ + lag2_, disturbance now includes the stock lag2
# columns). Dataset is chosen with V10_TUNE_DATADIR / V10_TUNE_PREFIX so the
# same objective serves tc>=30 and tc>=50.
#
# Usage:
...
SEED <- 42; NFOLD <- 5
datadir <- Sys.getenv("V10_TUNE_DATADIR", "derived_tables/outputs_afterEGU_results/v10_lag2")
prefix  <- Sys.getenv("V10_TUNE_PREFIX",  "v10_lag2")
MODELS  <- list(M2_24m = "C+D", M6_raw_24m = "C+D+M_raw")
...
df <- as.data.frame(fread(sprintf("%s/%s_B2_%s_harmonized.csv", datadir, prefix, resp)))
