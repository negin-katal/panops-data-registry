...
if (dataset_type == "filtered") {
  ...
  output_dir <- "plots/V10/sites_tc50/SHAP_percent"
...
# DRAFT: filter before setting levels - factor() alone would leave uWUE rows as NA
# and facet_wrap would render an empty NA panel
df_plot <- df_plot[df_plot$response %in% EFP_ORDER, ]
...
for (model in models) {
  ...
  for (method in methods) {
    ...
    ggsave(fname, combined, width = 190, height = 240, units = "mm", dpi = 300, bg = DARK_BG)
    cat(sprintf("  ✓ %s\n", basename(fname)))

    pdf_fname <- sub("\\.png$", ".pdf", fname)
    ggsave(pdf_fname, combined, width = 190, height = 240, units = "mm", bg = DARK_BG)
    cat(sprintf("  ✓ %s\n", basename(pdf_fname)))
  }
}
