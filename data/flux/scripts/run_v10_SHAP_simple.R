#!/usr/bin/env Rscript
# ============================================================================
# V10 Site-Level SHAP Values - SIMPLIFIED collection
# ============================================================================
...
if (length(args) < 1) {
  stop("Usage: Rscript run_v10_SHAP_simple.R <dataset_type>")
}

dataset_type <- args[1]

cat("\n", strrep("=", 80), "\n", sep="")
cat(sprintf("V10 SITE-LEVEL SHAP VALUES (SIMPLIFIED): %s\n", toupper(dataset_type)))
cat(strrep("=", 80), "\n\n", sep="")
...
# ============================================================================
# Main loop - collect all results in list, combine once at end
# ============================================================================
  ...
  for (benchmark in c('B1', 'B2')) {
    ...
    for (model_id in names(model_specs)) {
      ...
      shap_results <- mclapply(sites_test, function(site_hold) {
        ...
        # Return as list of rows for reliable combining
        lapply(seq_along(importance_norm), function(i) {
          list(
            variable = names(importance_norm)[i],
            mean_abs_shap = importance_norm[i],
            model = paste0(model_id, '_', tolower(benchmark)),
            response = response_var,
            test_site = site_hold,
            group = get_predictor_group(names(importance_norm)[i])
          )
        })
      }, mc.cores = n_cores_use)

      # Collect non-NULL results into list
      shap_results <- shap_results[!sapply(shap_results, is.null)]
      if (length(shap_results) > 0) {
        # Flatten: each site returns list of rows, unlist to get all rows
        flat_rows <- unlist(shap_results, recursive = FALSE)
        all_results_list <- c(all_results_list, flat_rows)
        result_count <- result_count + length(flat_rows)
      }
...
# ============================================================================
# Combine all results at the end and save
# ============================================================================

cat("\nSaving outputs...\n")
cat(sprintf("Total results collected: %d\n", result_count))

if (length(all_results_list) > 0) {
  # Convert list of rows to data.table
  all_results <- as.data.table(rbindlist(lapply(all_results_list, as.data.frame)))

  cat(sprintf("Combined data.table: %d rows\n", nrow(all_results)))

  shap_file <- sprintf('%s/RF_site_shap_M04_M08.csv', output_base)
  fwrite(all_results, shap_file)
  cat(sprintf("✓ %s (%d rows)\n", shap_file, nrow(all_results)))
} else {
  cat("ERROR: No results collected\n")
}
