#!/usr/bin/env Rscript
# ============================================================================
# Delta RMSE from adding disturbance - FOUR learners
#   RF (baseline) | RF exp02 (always.split raw memory) | XGBoost | LightGBM
...
#
#   1) deltaRMSE_4way.png         - mean dRMSE % per with-D/without-D pair
#   2) deltaRMSE_by_EFP_4way.png  - same, broken out by response
#   3) RMSE_paired_4way.png       - absolute RMSE without vs with D (normalised)
...
COL <- c("RF"="#4DAECC", "RF exp02"="#B07AFF", "XGBoost"="#F0A500", "LightGBM"="#7ED957")
LEV <- names(COL)
...
p1 <- ggplot(agg, aes(pair, dRMSE, fill=learner)) +
  ...
  labs(title="Delta RMSE from adding disturbance - RF vs RF exp02 vs XGBoost vs LightGBM",
...
ggsave(file.path(OUT,"deltaRMSE_4way.png"), p1, width=12.5, height=6.5, dpi=200, bg=DARK)
ggsave(file.path(OUT,"deltaRMSE_4way.pdf"), p1, width=12.5, height=6.5, bg=DARK)
...
ggsave(file.path(OUT,"deltaRMSE_by_EFP_4way.png"), p2, width=14, height=6.5, dpi=200, bg=DARK)
ggsave(file.path(OUT,"deltaRMSE_by_EFP_4way.pdf"), p2, width=14, height=6.5, bg=DARK)
...
p3 <- ggplot(pr, aes(pair, RMSE_rel, fill=learner, alpha=D)) +
  geom_col(position=position_dodge(0.82), width=0.74, colour="#FFFFFF", linewidth=0.18) +
  facet_grid(window ~ dataset) +
  scale_fill_manual(values=COL, name=NULL) +
  scale_alpha_manual(values=c("without D"=0.30, "with D"=1), name=NULL) +
  labs(title="LOSO RMSE without vs with disturbance, four learners",
  ...
  thm + theme(axis.text.x=element_text(size=6.5))
ggsave(file.path(OUT,"RMSE_paired_4way.png"), p3, width=12.5, height=6.5, dpi=200, bg=DARK)
ggsave(file.path(OUT,"RMSE_paired_4way.pdf"), p3, width=12.5, height=6.5, bg=DARK)
...
p4 <- ggplot(aggR, aes(pair, dR2, fill=learner)) +
  ...
  facet_grid(window ~ dataset) +
  scale_fill_manual(values=COL, name=NULL) +
  labs(title="Delta R2 from adding disturbance - RF vs RF exp02 vs XGBoost vs LightGBM",
  ...
  thm + theme(axis.text.x=element_text(size=6.5))
ggsave(file.path(OUT,"deltaR2_4way.png"), p4, width=12.5, height=6.5, dpi=200, bg=DARK)
ggsave(file.path(OUT,"deltaR2_4way.pdf"), p4, width=12.5, height=6.5, bg=DARK)

# ------------------------------------------- 5) dR2 per EFP, four learners --
p5 <- ggplot(long[dataset=="All sites (112)"], aes(pair, dR2, fill=learner)) +
  ...
  thm + theme(axis.text.x=element_text(angle=45, hjust=1, size=5.5))
ggsave(file.path(OUT,"deltaR2_by_EFP_4way.png"), p5, width=14, height=6.5, dpi=200, bg=DARK)
ggsave(file.path(OUT,"deltaR2_by_EFP_4way.pdf"), p5, width=14, height=6.5, bg=DARK)
...
cat("saved deltaRMSE_4way.png, deltaRMSE_by_EFP_4way.png, RMSE_paired_4way.png, deltaR2_4way.png, deltaR2_by_EFP_4way.png\n\n")
