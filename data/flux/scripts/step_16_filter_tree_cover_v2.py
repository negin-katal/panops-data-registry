#!/usr/bin/env python3
"""
Step 16: Filter RF-ready dataset to keep only sites with >=30% tree cover
"""
...
print("\n" + "="*80)
print("STEP 16: FILTER SITES BY TREE COVER (>=30%)")
print("="*80 + "\n")
...
# Identify sites to keep (>=30% tree cover)
sites_keep = tree_cover_per_site[tree_cover_per_site >= 30].index.tolist()
sites_remove = tree_cover_per_site[tree_cover_per_site < 30].index.tolist()

print(f"Sites with <30% tree cover (REMOVE): {len(sites_remove)}")
...
print(f"\nSites with >=30% tree cover (KEEP): {len(sites_keep)}\n")

# Filter dataset
df_filtered = df_rf[df_rf['SITE_ID'].isin(sites_keep)].reset_index(drop=True)

print("After filtering to >=30% tree cover:")
