...
export V10_OUT_ROOT="plots/V10/LGB_optuna"

DATASETS=${1:-"all_sites filtered"}

for DS in $DATASETS; do
  if [ "$DS" = "all_sites" ]; then
    export V10_IN_DIR="derived_tables/outputs_afterEGU_results/LGB_v10_all_sites_optuna"
  else
    export V10_IN_DIR="derived_tables/outputs_afterEGU_results/LGB_v10_optuna"
  fi
