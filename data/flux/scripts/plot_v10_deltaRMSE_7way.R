#!/usr/bin/env Rscript
# ============================================================================
# Delta RMSE from adding disturbance - SEVEN variants
...
#
#   1) deltaRMSE_7way.png         - mean dRMSE % per with-D/without-D pair
#   2) deltaRMSE_by_EFP_7way.png  - same, broken out by response
#   3) RMSE_paired_7way.png       - absolute RMSE without vs with D (normalised)
...
p1 <- ggplot(agg, aes(pair, dRMSE, fill=learner)) +
  ...
  geom_col(position=position_dodge(0.90), width=0.85) +
  facet_grid(window ~ dataset) +
  scale_fill_manual(values=COL, name=NULL) +
  labs(title="Delta RMSE from adding disturbance - seven variants: RF, exp01, exp02, XGBoost (+tuned), LightGBM (+tuned)",
...
ggsave(file.path(OUT,"deltaRMSE_7way.png"), p1, width=12.5, height=6.5, dpi=200, bg=DARK)
ggsave(file.path(OUT,"deltaRMSE_7way.pdf"), p1, width=12.5, height=6.5, bg=DARK)
...
ggsave(file.path(OUT,"deltaRMSE_by_EFP_7way.png"), p2, width=14, height=6.5, dpi=200, bg=DARK)
ggsave(file.path(OUT,"deltaRMSE_by_EFP_7way.pdf"), p2, width=14, height=6.5, bg=DARK)
...
p3 <- ggplot(pr, aes(pair, RMSE_rel, fill=learner, alpha=D)) +
  geom_col(position=position_dodge(0.90), width=0.85, colour="#FFFFFF", linewidth=0.18) +
  facet_grid(window ~ dataset) +
  scale_fill_manual(values=COL, name=NULL) +
  scale_alpha_manual(values=c("without D"=0.30, "with D"=1), name=NULL) +
  labs(title="LOSO RMSE without vs with disturbance, seven variants",
  ...
  thm + theme(axis.text.x=element_text(size=6.5))
ggsave(file.path(OUT,"RMSE_paired_7way.png"), p3, width=12.5, height=6.5, dpi=200, bg=DARK)
ggsave(file.path(OUT,"RMSE_paired_7way.pdf"), p3, width=12.5, height=6.5, bg=DARK)

# ------------------------------------------------ 4) dR2, seven variants -----
...
p4 <- ggplot(aggR, aes(pair, dR2, fill=learner)) +
  ...
  facet_grid(window ~ dataset) +
  scale_fill_manual(values=COL, name=NULL) +
  labs(title="Delta R2 from adding disturbance - seven variants: RF, exp01, exp02, XGBoost (+tuned), LightGBM (+tuned)",
  ...
  thm + theme(axis.text.x=element_text(size=6.5))
ggsave(file.path(OUT,"deltaR2_7way.png"), p4, width=12.5, height=6.5, dpi=200, bg=DARK)
ggsave(file.path(OUT,"deltaR2_7way.pdf"), p4, width=12.5, height=6.5, bg=DARK)

# ------------------------------------------- 5) dR2 per EFP, seven variants --
p5 <- ggplot(long[dataset=="All sites (112)"], aes(pair, dR2, fill=learner)) +
  ...
  thm + theme(axis.text.x=element_text(angle=45, hjust=1, size=5.5))
ggsave(file.path(OUT,"deltaR2_by_EFP_7way.png"), p5, width=14, height=6.5, dpi=200, bg=DARK)
ggsave(file.path(OUT,"deltaR2_by_EFP_7way.pdf"), p5, width=14, height=6.5, bg=DARK)
...
cat("saved deltaRMSE_7way.png, deltaRMSE_by_EFP_7way.png, RMSE_paired_7way.png, deltaR2_7way.png, deltaR2_by_EFP_7way.png\n\n")
