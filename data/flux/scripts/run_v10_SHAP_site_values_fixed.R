#!/usr/bin/env Rscript
# ============================================================================
# V10 Site-Level SHAP Values (FIXED - proper data collection)
# ============================================================================
...
if (length(args) < 1) {
  stop("Usage: Rscript run_v10_SHAP_site_values_fixed.R <dataset_type>")
}

dataset_type <- args[1]

cat("\n", strrep("=", 80), "\n", sep="")
cat(sprintf("V10 SITE-LEVEL SHAP VALUES (FIXED): %s\n", toupper(dataset_type)))
cat(strrep("=", 80), "\n\n", sep="")
...
for (response_var in response_vars) {
  ...
  for (benchmark in c('B1', 'B2')) {
    ...
    for (model_id in names(model_specs)) {
      ...
      # LOSO in parallel - return data.table directly
      shap_results <- mclapply(sites_test, function(site_hold) {
        ...
        # Return as data.table - one row per variable per site
        data.table(
          ...
          test_site = site_hold,
          group = as.character(sapply(names(importance_norm), get_predictor_group, USE.NAMES = FALSE))
        )
      ...
      if (length(shap_results) > 0) {
        ...
        if (length(shap_results) > 0) {
          all_results <- rbind(all_results, rbindlist(shap_results))
        }
      }
