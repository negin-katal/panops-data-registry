...
d <- fread(sprintf("%s/XGB_v10_optuna/XGB_site_signed_shap_M4.csv", B))
...
st <- dt[, .(p = tryCatch(wilcox.test(mean_signed_shap, mu=0)$p.value, error=function(e) NA_real_),
         ...
         by=.(response, window, metric, cat2)]
st[, q := p.adjust(p, "BH")]
st[, sig := ifelse(is.na(q),"",ifelse(q<0.001,"***",ifelse(q<0.01,"**",ifelse(q<0.05,"*","ns"))))]
yr <- dt[, .(lo=quantile(mean_signed_shap,0.02), hi=quantile(mean_signed_shap,0.98)), by=response]
st <- merge(st, yr, by="response")
...
for (w in c("12m","24m")) {
  ...
  p <- ggplot(sub, aes(x=metric, y=mean_signed_shap, fill=cat2)) +
    ...
    labs(x=NULL, y="Net signed disturbance SHAP  (response units)",
         ...
         subtitle=paste0("Tree cover ≥30 % (93 sites) · XGBoost–Optuna · net signed contribution of the ",
    ...
    th
  ggsave(file.path(OUT, sprintf("fig6_signed_shap_%s.png", w)), p, width=14, height=5.5, dpi=300, bg=DARK_BG)
  ggsave(file.path(OUT, sprintf("fig6_signed_shap_%s.pdf", w)), p, width=14, height=5.5, bg=DARK_BG)
}
fwrite(st[, .(response,window,metric,cat2,median=round(med,3),pct_negative=round(neg,1),
              p=signif(p,3), q=signif(q,3), sig)], "manuscript_coauthor_draft/fig6_statistics.csv")
cat("\nFig 6 saved (both windows)\n")
