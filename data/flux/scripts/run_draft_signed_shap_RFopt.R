...
TUNED <- local({
  R <- fread("plots/V10/Optuna/optuna_best_configs.csv")[learner == "RF"]
...
if (SMOKE) { RESPONSE_VARS <- c("GPPsat"); model_specs <- model_specs["M4_12m"] }
...
for (resp in RESPONSE_VARS) {
  ...
  for (model_id in names(model_specs)) {
    ...
    fold_shap <- mclapply(sites_test, function(test_site) {
      ...
      # then average over the site years. get_predictor_group also matches the sanitised
      # names, since the Traits patterns key on the "Leaf"/"Stem" prefixes either way.
...
if (length(shap_results) > 0) {
  ...
  out_path <- file.path(output_base,
    "RF_site_signed_shap_M4.csv")   # DRAFT: never the production file
