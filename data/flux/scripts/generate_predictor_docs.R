#!/usr/bin/env Rscript
# Generate predictor documentation for RF results
...
cat('\n====================================================================\n')
cat('GENERATING PREDICTOR DOCUMENTATION FOR RF RESULTS\n')
cat('====================================================================\n\n')
...
dt_b1 <- fread('derived_tables/outputs_afterEGU_results/V9_final_benchmarks/benchmark_B1_lag1only_final.csv')

trait_cols <- grep('^(P12|P50|P88|gsmax|rdmax)_mean', names(dt_b1), value=TRUE)
dist_cols_cur <- grep('^(forest_mean|deadwood_mean|mortality_intensity)_500m$', names(dt_b1), value=TRUE)
dist_cols_lag1 <- grep('^(forest_mean|deadwood_mean|mortality_intensity)_500m_lag1$', names(dt_b1), value=TRUE)
meteo_cols_cur <- grep('^(TA|VPD|P|SW_IN)_', names(dt_b1), value=TRUE)
meteo_cols_lag1 <- grep('^(TA|VPD|P|SW_IN)_.*_lag1$', names(dt_b1), value=TRUE)
meteo_cols_cur <- setdiff(meteo_cols_cur, meteo_cols_lag1)
...
model_configs_b1 <- list(
  M01 = list(predictors = c(meteo_cols_cur, meteo_cols_lag1),
             description = "Climate: current + lag1 meteorology"),
  M02 = list(predictors = c(meteo_cols_cur, meteo_cols_lag1, dist_cols_cur, dist_cols_lag1),
  ...
  M04 = list(predictors = c(meteo_cols_cur, meteo_cols_lag1, trait_cols, dist_cols_cur, dist_cols_lag1),
  ...
  M06_raw = list(predictors = c(meteo_cols_cur, meteo_cols_lag1, dist_cols_cur, dist_cols_lag1, efp_lag_raw),
             description = "Climate + Disturbance + Memory (raw)"),
  M06_anom = list(predictors = c(meteo_cols_cur, meteo_cols_lag1, dist_cols_cur, dist_cols_lag1, efp_lag_anom),
  ...
  M08_raw = list(predictors = c(meteo_cols_cur, meteo_cols_lag1, trait_cols, dist_cols_cur, dist_cols_lag1, efp_lag_raw),
             description = "Full + Memory (raw): Climate + Traits + Disturbance + raw EFP memory"),
  M08_anom = list(predictors = c(meteo_cols_cur, meteo_cols_lag1, trait_cols, dist_cols_cur, dist_cols_lag1, efp_lag_anom),
...
out_file_b1 <- 'derived_tables/outputs_afterEGU_results/RF_B1_lag1_final/PREDICTORS.csv'
...
dt_b2 <- fread('derived_tables/outputs_afterEGU_results/V9_final_benchmarks/benchmark_B2_lag1and2_final.csv')

trait_cols <- grep('^(P12|P50|P88|gsmax|rdmax)_mean', names(dt_b2), value=TRUE)
dist_cols_cur <- grep('^(forest_mean|deadwood_mean|mortality_intensity)_500m$', names(dt_b2), value=TRUE)
dist_cols_lag1 <- grep('^(forest_mean|deadwood_mean|mortality_intensity)_500m_lag1$', names(dt_b2), value=TRUE)
dist_cols_lag2 <- grep('^(forest_mean|deadwood_mean|mortality_intensity)_500m_lag2$', names(dt_b2), value=TRUE)
meteo_cols_cur <- grep('^(TA|VPD|P|SW_IN)_', names(dt_b2), value=TRUE)
meteo_cols_lag12 <- grep('^(TA|VPD|P|SW_IN)_.*_(lag1|lag2)$', names(dt_b2), value=TRUE)
meteo_cols_cur <- setdiff(meteo_cols_cur, meteo_cols_lag12)
...
model_configs_b2 <- list(
  M01 = list(predictors = c(meteo_cols_cur, meteo_cols_lag12),
             description = "Climate: current + lag1+lag2 meteorology"),
  M02 = list(predictors = c(meteo_cols_cur, meteo_cols_lag12, dist_cols_cur, dist_cols_lag1, dist_cols_lag2),
  ...
  M04 = list(predictors = c(meteo_cols_cur, meteo_cols_lag12, trait_cols, dist_cols_cur, dist_cols_lag1, dist_cols_lag2),
             description = "Full model: Climate + Traits + Disturbance"),
  ...
  M06_raw = list(predictors = c(meteo_cols_cur, meteo_cols_lag12, dist_cols_cur, dist_cols_lag1, dist_cols_lag2, efp_lag_raw),
             description = "Climate + Disturbance + Memory (raw)"),
  M06_anom = list(predictors = c(meteo_cols_cur, meteo_cols_lag12, dist_cols_cur, dist_cols_lag1, dist_cols_lag2, efp_lag_anom),
             description = "Climate + Disturbance + Memory (anomaly)"),
  ...
  M07_anom = list(predictors = c(meteo_cols_cur, meteo_cols_lag12, trait_cols, efp_lag_anom),
             description = "Climate + Traits + Memory (anomaly)"),
  M08_raw = list(predictors = c(meteo_cols_cur, meteo_cols_lag12, trait_cols, dist_cols_cur, dist_cols_lag1, dist_cols_lag2, efp_lag_raw),
             description = "Full + Memory (raw): Climate + Traits + Disturbance + raw EFP memory"),
  M08_anom = list(predictors = c(meteo_cols_cur, meteo_cols_lag12, trait_cols, dist_cols_cur, dist_cols_lag1, dist_cols_lag2, efp_lag_anom),
             description = "Full + Memory (anomaly): Climate + Traits + Disturbance + EFP anomaly")
)
...
out_file_b2 <- 'derived_tables/outputs_afterEGU_results/RF_B2_lag1and2_final/PREDICTORS.csv'
...
cat('B1 (Lag1) Models:\n')
print(df_doc_b1[, .(Model, Description, N_Predictors)])

cat('\n\nB2 (Lag1+Lag2) Models:\n')
print(df_doc_b2[, .(Model, Description, N_Predictors)])

cat('\n✅ PREDICTOR DOCS CREATED!\n')
cat('====================================================================\n\n')
