#!/usr/bin/env Rscript
# ============================================================================
# V10 All-Sites Plot: Bar chart - Number of sites per IGBP class
# ============================================================================
...
# ============================================================
# Load v10 all-sites data to get IGBP info per site
# ============================================================

cat("Loading V10 all-sites data...\n")
...
# Load v10 all-sites harmonized data to get sites used
df_b1_all <- fread('derived_tables/outputs_afterEGU_results/v10_all_sites/v10_all_B1_GPPsat_harmonized.csv')
v10_all_sites <- unique(df_b1_all$SITE_ID)

# Get unique sites with their IGBP (only v10 all-sites sites)
site_dt <- df_main[SITE_ID %in% v10_all_sites][, .(IGBP = first(IGBP)), by = SITE_ID]
...
p_bar <- ggplot(bar_dt, aes(x = reorder(IGBP_f, -N), y = N, fill = IGBP)) +
  ...
  labs(title = sprintf("Sites per IGBP class - All sites (n = %d total)", nrow(site_dt)),
...
ggsave(file.path(out_dir, "bar_sites_per_IGBP_all_sites.png"),
       p_bar, width = 8, height = 4.5, dpi = 200)
ggsave(file.path(out_dir, "bar_sites_per_IGBP_all_sites.pdf"),
       p_bar, width = 8, height = 4.5)

cat(sprintf("✓ Saved: %s/bar_sites_per_IGBP_all_sites.png\n", out_dir))
