...
# =========================================================
# LEAVE-ONE-SITE-OUT RF — WET SITES WITH LOW FOREST COVER REMOVED
#
# Identical to run_18_RF_LOSO.R except WET IGBP sites whose
# mean forest cover (500 m buffer, averaged across all years)
# is < 20 % are excluded before model fitting.
#
# Threshold: forest_mean_pct_500m site-mean < 20
# Output goes to RF_outputs_noWET/ so all runs coexist.
# =========================================================
...
out_dir    <- "derived_tables/outputs_afterEGU_results/RF_outputs_noWET"
...
# =========================================================
# FILTER: remove WET sites with mean forest cover < 20 %
# (500 m buffer averaged across all site-years)
# =========================================================

FOREST_COL    <- "forest_mean_pct_500m"
FOREST_THRESH <- 20.0

site_forest <- dt[IGBP == "WET",
                  .(mean_forest = mean(get(FOREST_COL), na.rm = TRUE)),
                  by = SITE_ID]
sites_to_drop <- site_forest[mean_forest < FOREST_THRESH, SITE_ID]

cat(sprintf("\nWET sites with mean %s < %.0f%% → removed (%d sites):\n",
            FOREST_COL, FOREST_THRESH, length(sites_to_drop)))
print(site_forest[SITE_ID %in% sites_to_drop][order(mean_forest)])
